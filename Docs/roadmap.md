...
- [ ] Electron のシステムトレイ常駐設定
- [ ] グローバルホットキーで呼び出し（`Ctrl+Shift+Space` などキー押下で録音開始）
...
| 動作方式 | バックエンドで常時マイクをモニタリング → 検出時に既存の STT→LLM→TTS パイプラインを起動 |
...
1. Electron でシステムトレイアイコンを表示する
2. グローバルホットキー（`globalShortcut`）で会話ウィンドウを呼び出す
3. ウェイクワード検出を実装する（OpenWakeWord をバックエンドに組み込み）
4. バックエンドの自動起動スクリプトを整備する（WSL2 起動時に自動で `uvicorn` 起動）
5. 会話履歴を SQLite または JSON ファイルで永続化する
6. ルートエージェントの専門エージェント振り分けロジックを設計する
